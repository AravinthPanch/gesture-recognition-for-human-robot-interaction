...
void DrawSkeleton(nite::UserTracker* pUserTracker, const nite::UserData& userData, int frameId)
{
    ...
    send_skeleton(userData, frameId);
}
...
// 					}
	...
	for (int i = 0; i < users.getSize(); ++i)
	{
		...
		else if (!user.isLost())
		{
			...
			{
				DrawSkeleton(m_pUserTracker, user, userTrackerFrame.getFrameIndex());
			}
		}
